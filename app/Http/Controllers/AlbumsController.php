...
class AlbumsController extends Controller
{
    ...
    {
        $albumMetadata = new Album();
        $data = array();

        // compile the table headers
        $data['headers'][] = 'id';
        $fillables = $albumMetadata->getFillable();
        foreach ($fillables as $fillable) {
            $data['headers'][] = $fillable;
        }

        // compile the table rows
        $albumItems = Album::all(); // returns $this->items
        foreach ($albumItems as $albumItem) {
            $data['rows'][] = $albumItem->getAttributes();
        }

        // todo: carry on from here
        return view('album/album_list_view', ['data' => $data]);
    }
